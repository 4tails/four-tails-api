...
namespace FourTails.Api.Validations;

public class UserRegistrationDTOValidation : AbstractValidator<UserRegistrationDTO>
{
    public UserRegistrationDTOValidation()
    {
        ...
        RuleFor(user => user.Email).Custom((str, context) => 
        {
            if (str == null || str.Length == 0)
            {
                context.AddFailure("Email field should not be empty.");
            }
            else if (!Regex.Match(str, @"^[^@\s]+@[^@\s]+\.[^@\s]+$", RegexOptions.IgnoreCase).Success)
            {
                context.AddFailure("Email should folow specific pattern: username@example.com");
            } 
            else if (str.Length > 100)
            {
                context.AddFailure("Email should not exceed maximum of 100 characters.");
            }
        });
